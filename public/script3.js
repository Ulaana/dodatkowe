...
// Funkcja do ładowania danych z lokalnych plików
async function loadLocalData() {
    try {
        const [operatorData, bazyData, stacjeData, punktyData, slownikData] = await Promise.all([
            fetch('/json/operator.json').then(response => response.json()),
            fetch('/json/bazy.json').then(response_1 => response_1.json()),
            fetch('/json/stacje.json').then(response_2 => response_2.json()),
            fetch('/json/punkty.json').then(response_3 => response_3.json()),
            fetch('/json/slownik.json').then(response_4 => response_4.json())
        ]);
        return { operator: operatorData, bazy: bazyData, stacje: stacjeData, punkty: punktyData, slownik: slownikData };
    } catch (error) {
        console.error('Błąd podczas wczytywania lokalnych danych:', error);
    }
}

// Pobieranie danych z API lub lokalnych plików w razie niepowodzenia
async function fetchData() {
    try {
        const response = await fetch('/api/data');
        if (!response.ok) {
            throw new Error('Błąd serwera, wczytywanie lokalnych danych.');
        }
...
// Główna logika po pobraniu danych z API lub lokalnych plików
fetchData().then(data => {
    processData(data);
});

// Funkcja do przetwarzania danych - zarówno z API, jak i lokalnych plików
function processData(data) {
    ...
    slownik.charging_mode.forEach(mode => {
    ...
    punkty.data.forEach(station => {
        var matchingStacja = stacje.data.find(stacja => stacja.id === station.station_id);
        var matchingBaza = bazy.data.find(baza => baza.id === matchingStacja.pool_id);
        var matchingOperator = operator.data.find(operator => operator.id === matchingBaza.operator_id);
    ...
    // Obsługa filtra dla typów ładowania
